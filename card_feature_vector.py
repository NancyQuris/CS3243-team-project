class CardFeatureVectorCompute:
    ...
    def fetch_feature_essential(self, hole, community):
        f = self.fetch_feature(hole, community)
        f_val_comb = f[3] + f[4] + f[5] + f[6] + f[7]
        f_suit_comb = f[9] + f[10] + f[11] + f[12] + f[13]
        f_seq_comb = f[15] + f[16] + f[17]
        return [f[0], f[1], f[2], f_val_comb, f[8], f_suit_comb, f[14], f_seq_comb]
    ...
    def seq_count(self, cards):
        ...
        for card in cards:
            mask = 1 << card.rank
            memo |= mask
        s_len = self.__count_longest_seq(memo)
