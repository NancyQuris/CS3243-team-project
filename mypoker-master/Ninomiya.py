...
class RTPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        print(card_feature)
        # print(card_strength)
        card_strength_arr = np.array([card_feature[i] * self.thf.get_strength(self.street_idx)[i] for i in range(len(card_feature))])
        print(card_strength_arr)
        ...
        feature_vec = self.phi(card_strength_arr, my_stack, opp_stack, my_bet, opp_bet, my_total_gain)
        print("Nino!")
        print(feature_vec)
        self.feature_vector = feature_vec

        # value for taking different action
        self.pp.pprint(self.step_theta[self.street_idx])
    ...
    def phi(self, hand_strength_arr, my_stack, opp_stack, my_bet, opp_bet, my_total_gain):
        return np.array(
            [1] +
            list(hand_strength_arr) +
